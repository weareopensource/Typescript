 ...
 * get Joi result
 */
export function getResultFromJoi(body, schema, options) {
 ...
 */
export function isValid(schema): (req: Request, res: Response, next: NextFunction) => void {
  return (req, res, next) => {
    ...
    if (_.includes(config.joi.supportedMethods, method)) {
      ...
      if (errors) return error(res, 422, 'Schema validation error', errors)(result.error);
