...
function checkError(result) {
    if (result && result.error) {
        if (result.error.original && (result.error.original.password || result.error.original.firstname))
            result.error.original = lodash_1.default.pick(result.error.original, config_1.default.whitelists.users.default);
        let description = '';
        ...
        if (result.error._original && (result.error._original.password || result.error._original.firstname))
            result.error._original = lodash_1.default.pick(result.error._original, config_1.default.whitelists.users.default);
        return description;
    }
    return false;
}
...
function isValid(schema) {
    return (req, res, next) => {
        ...
        const options = lodash_1.default.clone(config_1.default.joi.validationOptions);
        if (lodash_1.default.includes(config_1.default.joi.supportedMethods, method)) {
            if (method === 'put') {
            ...
            if (errors)
                return responses_1.error(res, 422, 'Schema validation error', errors)(result.error);
