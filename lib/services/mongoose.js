...
async function loadModels(callback) {
    ...
    await Promise.all(config_1.default.files.mongooseModels.map(async (modelPath) => {
        await Promise.resolve().then(() => tslib_1.__importStar(require(path_1.default.resolve(modelPath))));
    }));
    if (callback)
        callback();
}
...
const connect = async () => {
    try {
        ...
        await mongoose_1.default.connect(config_1.default.db.uri, { user: config_1.default.db.options.user, pass: config_1.default.db.options.pass });
