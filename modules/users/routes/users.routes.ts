...
import { deleteUser, getMe, terms, update } from '../controllers/users/users.profile.controller';
import usersSchema from '../models/user.schema';
import * as usersData from '../controllers/users.data.controller';

export default (app) => {
  app.route('/api/users/me').get(passport.authenticate('jwt'), policy.isAllowed, getMe);

  app.route('/api/users/terms').get(passport.authenticate('jwt'), policy.isAllowed, terms);

  app.route('/api/users').all(passport.authenticate('jwt'), policy.isAllowed).put(model.isValid(usersSchema), update).delete(deleteUser);
